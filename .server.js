...
app.post("/upload", upload.single("file"), async (req, res) => {
  ...
  const result = await Promise.all(
    json.map(async (column) => {
      ...
      const {
        window: { document },
      } = await JSDOM.fromURL(page).catch(err => console.log(err));
      ...
      // const testUrl =
      //   "/on/demandware.static/-/Sites-acne-product-catalog/default/dweddd43ca/images/AL/AL0239-/1500x/AL0239-CRY_C.jpg";
